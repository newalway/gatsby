...
class AdminPageAboutUsController extends Controller
{
    const ROUTER_INDEX = 'admin_page_about_us';
    const ROUTER_CONTROLLER = 'AdminPageAboutUs';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'about_us']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('About Us');
            $data->translate('th')->setTitle('เกี่ยวกับเรา');
            $data->setName('about_us');
            $em->persist($data);
