...
class AdminPagePortfolioController extends Controller
{
    const ROUTER_INDEX = 'admin_page_portfolio';
    const ROUTER_CONTROLLER = 'AdminPagePortfolio';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'portfolio']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('Portfolio');
            $data->translate('th')->setTitle('ผลงาน');
            $data->setName('portfolio');
            $em->persist($data);
