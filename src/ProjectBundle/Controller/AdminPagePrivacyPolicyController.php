...
class AdminPagePrivacyPolicyController extends Controller
{
    const ROUTER_INDEX = 'admin_privacy_policy';
    const ROUTER_CONTROLLER = 'AdminPagePrivacyPolicy';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'privacy_policy']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('Privacy Policy');
            $data->translate('th')->setTitle('นโยบายความเป็นส่วนตัว');
            $data->setName('privacy_policy');
            $em->persist($data);
