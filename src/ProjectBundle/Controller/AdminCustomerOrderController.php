...
class AdminCustomerOrderController extends Controller
{
	...
	*/
	public function indexAction(Request $request)
	{
		...
		$query = $repository->findCustomerOrderHasItem($arr_query_data);
  	...
  	public function order_summary_reportExcelAction(request $request)
	{
		...
		$arr_query_data = $this->prepare_query_data($request);
		$orders = $em->getRepository(CustomerOrder::class)->findCustomerOrderHasItem($arr_query_data)->getQuery()->getResult();

		$export_excel = $this->container->get('exportexcel');
		$export_excel->getHeaderExcelOrder();
		$export_excel->setDataExcelOrder($orders);
		$export_excel->setStyleAndFormatExcelOrder();
	...
	*/
	public function viewAction(CustomerOrder $customer_orderObj,Request $request)
	{
		...
		// 	$delivery_method = DeliveryMethodQuery::create()->findOne();
		// 	$non_delivery_dates = $delivery_method->getNonDeliveryDate();
		// 	if(count($non_delivery_dates)>0){
		...
		// 	//get holidays
		// 	$arr_holidays=array();
		// 	$holidays = HolidayQuery::create()->setPublicData()->find();
		// 	if($holidays){
		// 		foreach ($holidays as $key_hol => $holiday) {
		// 			$arr_holidays[$key_hol] = $holiday->getHolidayDate()->format('j-n-Y');
		// 		}
		// 	}
		// 	//get payment
	...
	*/
	public function updateStatusAction(CustomerOrder $customer_orderObj,Request $request)
	{
		...
		if ($req_status == $payment_status_paid){
			...
			$this->get('session')->getFlashBag()->add('notice', 'paid status update succes');

		}elseif($req_status == $payment_status_shipped){
			$customer_orderObj->setFulfilled(1);
			$em->flush($customer_orderObj);
			$this->get('session')->getFlashBag()->add('notice', 'shipped status update succes');

		}elseif($req_status == $payment_status_cancelled){
			$customer_orderObj->setCancelled(1);
			$em->flush($customer_orderObj);
			$this->get('session')->getFlashBag()->add('notice', 'cancelled update succes');
			...
			$em->flush($customer_orderObj);
			$this->get('session')->getFlashBag()->add('notice', 'deleted update succes');
	...
	*/
	public function _getunreadAction(request $request)
	{
		...
		$count = $em->getRepository(CustomerOrder::class)->countUnreadCustomerOrder()
				->select('count(o.id)')
				->getQuery()
