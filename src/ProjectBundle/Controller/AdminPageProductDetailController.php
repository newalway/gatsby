...
class AdminPageProductDetailController extends Controller
{
	const ROUTER_INDEX = 'admin_page_product_detail';
    const ROUTER_CONTROLLER = 'AdminPageProductDetail';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'product_detail']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('Product Detail');
            $data->translate('th')->setTitle('Product Detail');
            $data->setName('product_detail');
            $em->persist($data);
