...
class AdminPageStyleGuideController extends Controller
{
    const ROUTER_INDEX = 'admin_page_style_guide';
    const ROUTER_CONTROLLER = 'AdminPageStyleGuide';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'style_guide']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('Style Guide');
            $data->translate('th')->setTitle('Style Guide');
            $data->setName('style_guide');
            $em->persist($data);
