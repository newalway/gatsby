...
use ProjectBundle\Form\Type\AdminShowroomType;
...
{
	const ROUTER_INDEX = 'admin_showroom';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  	const ROUTER_PREFIX = 'showroom';
	const ROUTER_CONTROLLER = 'AdminShowroom';

	protected function getQuerySearchData($arr_query_data)
	{
		$repository = $this->getDoctrine()->getRepository(Showroom::class);
	...
	*/
	public function newAction(Request $request)
	{
		...
		$data = new Showroom();
		$form = $this->createForm(AdminShowroomType::class, $data);
	...
	*/
	public function editAction(Request $request)
	{
		...
		$data = $this->getDoctrine()->getRepository(Showroom::class)->find($request->get('id'));
	...
	*/
	public function deleteAction(Request $request)
	{
		...
		$data = $em->getRepository(Showroom::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(Showroom::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
				    $em->flush();
	...
	*/
	public function getGoogleMapsApiKey()
	{
		...
		$data = $em->getRepository(Authentication::class)->findOneBy(
			array('name'=>'google_maps_api_key'), array('id' => 'DESC')
		);
		return $data;
	}
