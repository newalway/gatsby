...
class Products
{
			...
			if ($skus) {
				foreach ($skus as $key => $sku) {
					...
					$tmp_data = array(
						...
						"price"	=> $sku->getPrice(),
						"compare_at_price"	=> $sku->getCompareAtPrice(),
						"inventory_policy_status"=> $sku->getInventoryPolicyStatus(),
	...
	public function saveProductImageTechnique(Product $product)
	{
		$em = $this->container->get('doctrine')->getEntityManager();
		$request = $this->container->get('request_stack')->getCurrentRequest();
...
// 		productTechNique
		...
		//dump($product_tech);
		//exit;
		$product_tech = $product->getProductTechNique();
		$product_in_product_tech = $em->getRepository(ProductTechNique::class)->findByProduct($product);
		 if(count($product_in_product_tech) > 0){
			 foreach ($product_in_product_tech as $caption_image_old) {
				 $em->remove($caption_image_old);
			 }
			 foreach ($product_tech as $caption_image) {
				 //$aa = new ProductTechNique();
				 $caption_image->setProduct($product);
				 $em->persist($caption_image);
			 }
			 $em->flush();
		...
		// dump($product);//productTechNique
		// exit;
		// if(count($product_tech) > 0){
		// 	$product_in_product_tech = $em->getRepository(ProductTechNique::class)->findByProduct($product);
		// 	$count_product_in_product_tech = count($product_in_product_tech);
		// 	if($count_product_in_product_tech > 0) {
		//
		// 	}
		// 	// dump($product_tech);
		// 	//$result = array_diff($caption_image, $product_in_product_tech);
		//
		// 	//print_r($result);
		//
		//
		...
		// 		$em->persist($caption_image);
		// 	}
		// 	$em->flush($caption_image);
		// }

		// //delete image_gallery
		// if(!empty($del_img_gallery_ids)){
		// 	foreach ($del_img_gallery_ids as $del_img_id) {
		// 		$del_style_image = $em->getRepository(ProductTechNique::class)->find($del_img_id);
		// 		$em->remove($del_style_image);
		// 	}
		// 	$em->flush();
		// }
		...
		// 	foreach ($image_path as $image_uri) {
		// 		if($image_uri){
		// 			$gallery_image = new ProductTechNique();
		// 			$gallery_image->setImage($image_uri);
		// 			$gallery_image->setProduct($product);
		// 			$em->persist($gallery_image);
		// 		}
	...
	public function getDeliveryDate()
	{
		...
		if($delivery_method){
			...
			if($delivery_method->getStatus()){
				...
				// $BKKTZ = new \DateTimeZone('Asia/Bangkok');
...
// $product_id = 9999;
					...
					if($sku_id){
						...
						if($sku){
							...
							$imageSmall = $sku->getImageSmall();
							$imageMedium = $sku->getImageMedium();
							$price = $sku->getPrice();
			...
			//get shipping rate
			$shipping_cost = $this->getDeliveryCostByCondition($arr_summary);
			$arr_summary['shipping_cost'] = $shipping_cost;
...
// exit;
	...
	public function getDeliveryCostByCondition($arr_summary)
	{
		...
		$item_count_total = $arr_summary['item_count'];
		$sub_total = $arr_summary['sub_total'];
		$weight_grams_total = $arr_summary['weight_grams_total'];

		$shipping_cost = 0;
		$is_match_free_shipping=false;

		$shipping_item_based_rate = $this->container->getParameter('shipping_item_based_rate');
		...
		if($shipping_rates){
			foreach ($shipping_rates as $shipping_rate) {
				...
				$rate_amount = ($shipping_rate->getRateAmount()) ? $shipping_rate->getRateAmount() : 0 ;
				$free_shipping = $shipping_rate->getFreeShipping();
				$is_match=false;

				switch ($rate_type) {
					case $shipping_item_based_rate:
						//item_based_rate
						if(($item_count_total>=$minimum_range)){
							if($maximum_range==0){
								//no limit for maximum
								$is_match=true;
							}else{
								if($item_count_total<=$maximum_range){
									$is_match=true;
								}
							}
						}
	...
	public function setSessionOrderSuccess($order)
	{
		$request = $this->container->get('request_stack')->getCurrentRequest();
		$session = $request->getSession();
		$session_cart = $this->getCartSession();
	...
	public function saveCustomerOrderItemAndUpdateInventory($customer_order, $arr_product)
	{
		$em = $this->container->get('doctrine')->getEntityManager();
