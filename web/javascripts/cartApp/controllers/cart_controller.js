app.controller('cartCtrl',['$scope', '$sce', 'product_data', function($scope, $sce, product_data){
	...
	$scope.getRouteProductDetail = function(product_id, product_title, variant) {
		if (!Array.isArray(variant) || !variant.length) {
			// array does not exist, is not an array, or is empty
			var link_product_detail = Routing.generate('product_detail', { id:product_id, slug:slugify(product_title) });
		}else{
			var str_variant = encodeURI(variant.join("-"));
			var link_product_detail = Routing.generate('product_detail', { id:product_id, slug:slugify(product_title), v:str_variant });
		}
